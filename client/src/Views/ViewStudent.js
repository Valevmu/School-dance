...
const ViewStudent = () => {
  ...
  const getStudentsFromService = async () => {
    try {
      const viewStudentResponse = await axios.get(`http://localhost:8080/api/students/${id}`, {withCredentials: true});
  ...
  const removeStudent = async (id) => {
    try{
      const response = await axios.delete(`http://localhost:8080/api/students/delete/${id}`, {withCredentials:true})
    ...
    <div className={styles['container']}>
     ...
     {students.students && (
      <div className={styles["data"]} key={students.id}  >
         ...
         className={styles['btn-delete']}>Eliminar inscripción</button>   
    </div>
    </div>
  );
};
